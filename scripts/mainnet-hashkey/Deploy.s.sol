...
contract Deploy is Script, DeployScript, Validator, EventValidator {
    function run() external {
        ...
        curators[0] = DeployConstants.HASHKEY_WSTETH_MULTISIG;

        string[] memory names = new string[](n);
        names[0] = DeployConstants.HASHKEY_VAULT_NAME;

        string[] memory symbols = new string[](n);
        symbols[0] = DeployConstants.HASHKEY_VAULT_SYMBOL;

        uint256[] memory maximalTotalSupplies = new uint256[](n);
        maximalTotalSupplies[0] = DeployConstants.MAXIMAL_TOTAL_SUPPLY_HASHKEY;

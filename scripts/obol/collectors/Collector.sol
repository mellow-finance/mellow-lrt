...
contract Collector is DefaultAccessControl {
    ...
    function collect(
        ...
        for (uint256 i = 0; i < n; i++) {
...

            {
                ...
                responses[i].maximalTotalSupplyWSTETH = ILidoWSteth(wsteth)
                    .getWstETHByStETH(responses[i].maximalTotalSupplyETH);
                responses[i].totalValueWSTETH = ILidoWSteth(wsteth)
    ...
    function convertTokenIntoEth(
        ...
        if (r.vault != address(0))
            return Math.mulDiv(amount, r.totalValueETH, r.totalSupply);
        if (token == weth || token == steth) return amount;
        if (token == wsteth)
            return ILidoWSteth(wsteth).getStETHByWstETH(amount);
        // by default == 1
        return amount;
    }
    ...
    function convertBaseTokenPriceToWethPrice(
        ...
        if (token == wsteth) {
            return ILidoWSteth(wsteth).getStETHByWstETH(priceX96);
        } else {

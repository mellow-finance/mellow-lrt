...
contract Integration is Test, DeployScript {
    ...
    function simplifyDepositSecurityModule(
        ...
        {   
            ...
            IStakingRouter.StakingModule memory module = IStakingRouter(router).getStakingModule(
    ...
    // forge test -vvv --fork-url $(grep HOLESKY_RPC .env | cut -d '=' -f2,3,4,5) --match-path ./tests/holesky-obol/ObolTestV2.t.sol  --evm-version shanghai --fork-block-number 1881344
    function testObol() external {
        ...
        IVaultConfigurator configurator = setup.configurator;
        ...
        deal(DeployConstants.WETH, address(setup.vault), amount);
